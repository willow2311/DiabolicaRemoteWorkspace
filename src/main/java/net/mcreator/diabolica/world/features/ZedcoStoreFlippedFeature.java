...
public class ZedcoStoreFlippedFeature extends Feature<NoneFeatureConfiguration> {
	...
	public ZedcoStoreFlippedFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("diabolica", "zedco_store_flipped"));
		if (template == null)
			return false;
